...
impl Normal {
    ...
    pub fn algebraic(a: &BigInt, b: &BigInt, poly: &Polynomial) -> BigInt {
        let a_d = BigRational::from_integer(a.clone());
        let b_d = BigRational::from_integer(b.clone());
        let ab = -a_d / b_d;
        let left = Self::polynomial_evaluate_big_rational(poly, &ab);
        let right = (-b).pow(poly.degree as u32);
    ...
    fn polynomial_evaluate_big_rational(polynomial: &Polynomial, indeterminate_value: &BigRational) -> BigRational {
        ...
        let mut result = BigRational::from_integer(polynomial.coefficients[num].clone());

        while num > 0 {
            num -= 1;
            result *= indeterminate_value;
            result += BigRational::from_integer(polynomial.coefficients[num].clone());
        }
